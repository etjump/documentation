...
```{note}
You may not set the value of this cvar to `maps`. This is to prevent default mapscripts from being "detected" as custom mapscripts. If you try to set `maps` as the directory, the value will reset to default.
```
