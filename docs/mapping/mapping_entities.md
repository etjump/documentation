...
tracker_gt       | [index,value] format specified above. | 1,-1 | Tracker activates targets if player's tracker value is greater than the specified value.
...
tracker_inc_if   | [index,value] format specified above. | 1,0 | Tracker increases player's target value by the specified value if `tracker_eq` matches the player's tracker value.
